...
**Program:** NextGen Learners — Data Science Track
...
1. **Import & Inspect** — Loaded the CSV and checked shape, data types, missing values, and duplicates using `.info()`, `.isnull().sum()`, and `.duplicated().sum()`.
...
6. **Final Quality Check** — Verified no missing values in key fields, no duplicate rows, and consistent formatting.
7. **Export** — Saved the cleaned dataset as `applicants_cleaned.csv`.
