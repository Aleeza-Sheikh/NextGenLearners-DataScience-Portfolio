...
The 5 SQL queries
How many interns completed each domain's program?
What is the average hackathon score per domain?
Which interns scored above a threshold (top performers)? — uses JOIN
Conversion rate from applied → completed, per domain — uses LEFT JOIN
Dropout vs completed interns, per domain
Dashboard
